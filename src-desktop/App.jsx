...
function AppContent() {
  ...
  const heroParallaxY = useTransform(scrollYProgress, [0, 0.42], [0, -18]);
  ...
  useEffect(() => {
    const direction = getTabDirection(previousTabRef.current, activeTab);
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      setPaymentProgress(44);
      await new Promise(resolve => setTimeout(resolve, 900));
      setPaymentProgress(76);
      await new Promise(resolve => setTimeout(resolve, 1400));
      setPaymentProgress(94);
      ...
      <div className="flex-1 flex flex-col pb-24 desktop-page-shell desktop-home-shell">
        ...
        <header className="p-6 flex justify-between items-center bg-white/50 backdrop-blur-md sticky top-0 z-20 desktop-page-header">
              ...
              {showLocationPicker && (
                <>
                  ...
                  <motion.div
                    ...
                    animate="animate"
                    exit="exit"
                    className="safe-modal viewport-center p-6"
                  >
                    <div className="flex justify-between items-center mb-6">
                    ...
                    <motion.div
                      ...
                      animate="animate"
                      className="space-y-3 mb-6 max-h-96 overflow-y-auto scrollbar-thin"
                    >
          ...
          <div className="flex gap-3">
            ...
            >
              <motion.span
                variants={iconLoopVariants}
                animate={!shouldReduceMotion && notifications.length > 0 ? 'active' : 'idle'}
              >
                <Bell size={18} />
              </motion.span>
            ...
            >
              <AppAvatar
                src={firebaseProfile?.avatar_url || profile?.avatar_url || ''}
                name={firebaseProfile?.full_name || profile?.full_name}
                email={profile?.email}
                size="md"
                className="w-10 h-10 border-0 shadow-none"
              />
            </motion.button>
          </div>
        </header>
              ...
              >
                ...
                {notifications.length === 0 ? (
                  <div className="p-8 text-center">
                    <motion.div
                      variants={iconLoopVariants}
                      animate={shouldReduceMotion ? 'idle' : 'active'}
            ...
            >
              <AnimatePresence mode="wait" custom={tabDirection}>
                <motion.div
                  key={activeTab}
                  custom={tabDirection}
                  variants={pageDirectionalVariants}
                  initial={shouldReduceMotion ? false : 'initial'}
                  animate="animate"
                  exit="exit"
                  className="flex-1 flex flex-col"
              ...
              <div className="desktop-detail-content flex-1 bg-white rounded-t-[3rem] -mt-10 p-8 relative z-10 shadow-2xl overflow-y-auto">
                ...
                <div className="mt-auto pt-6 flex flex-col gap-4 sticky bottom-0 bg-white/80 backdrop-blur-lg pb-6 px-2 -mx-2">
                  <div className="flex items-center justify-between">
                    ...
                    <motion.button
                      variants={anticipatoryButtonVariants}
                      initial="rest"
                      animate={shouldReduceMotion ? 'rest' : 'nudge'}
                      whileTap={{ scale: 0.94 }}
            ...
            <nav className="ios-tabbar desktop-tabbar flex items-center">
              ...
              >
                <motion.span
                  ...
                  variants={iconLoopVariants}
                  animate={!shouldReduceMotion && activeTab === 'home' ? 'active' : 'idle'}
                >
              ...
              {(isOwner || isAdmin) && (
                ...
                >
                  <motion.span
                    className="flex-shrink-0"
                    variants={iconLoopVariants}
                    animate={!shouldReduceMotion && activeTab === 'dashboard' ? 'active' : 'idle'}
                  >
              ...
              >
                <motion.span
                  className="flex-shrink-0"
                  variants={iconLoopVariants}
                  animate={!shouldReduceMotion && activeTab === 'bookings' ? 'active' : 'idle'}
                >
              ...
              >
                <motion.span
                  className="flex-shrink-0"
                  variants={iconLoopVariants}
                  animate={!shouldReduceMotion && activeTab === 'feedback' ? 'active' : 'idle'}
                >
              ...
              >
                <motion.span
                  className="flex-shrink-0"
                  variants={iconLoopVariants}
                  animate={!shouldReduceMotion && activeTab === 'calendar' ? 'active' : 'idle'}
                >
              ...
              >
                <motion.span
                  className="flex-shrink-0"
                  variants={iconLoopVariants}
                  animate={!shouldReduceMotion && activeTab === 'profile' ? 'active' : 'idle'}
                >
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center p-6 bg-zinc-900 rounded-[2rem] border border-white/5">
                        ...
                        <motion.button
                          ...
                          variants={anticipatoryButtonVariants}
                          initial="rest"
                          animate={selectedSlot && !shouldReduceMotion ? 'nudge' : 'rest'}
                        >
                          Continue
                        </motion.button>
                      </div>
            ...
            >
              <div className="w-full max-w-sm space-y-8">
                <div className="flex justify-center">
                  <div className="relative">
                    <motion.div
                      ...
                      className="w-24 h-24 rounded-full border-4 border-zinc-100 border-t-emerald-500"
                    />
                ...
                <div className="p-6 bg-zinc-50 rounded-3xl border border-zinc-100 space-y-4">
                  ...
                  <div className="pt-2">
                    <div className="flex justify-between items-center mb-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">
                      <span>Payment Progress</span>
                      <span>{clampProgress(paymentProgress)}</span>
                    </div>
                    <div className="h-2 w-full rounded-full bg-zinc-200 overflow-hidden">
                      <motion.div
                        className="h-full bg-emerald-500"
                        initial={{ width: '0%' }}
                        animate={{ width: clampProgress(paymentProgress) }}
